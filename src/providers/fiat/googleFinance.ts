...
export async function getGoogleFinanceRate(base: string, quote: string): Promise<ExchangeRateResult> {
  try {
    const r = await fetch(`https://www.google.com/finance/quote/${base.toUpperCase()}-${quote.toUpperCase()}`, {
      headers: {
    ...
    idx = data.indexOf('data-last-price="');
    if (idx !== -1) {
      data = data.substring(idx + 17);
      idx = data.indexOf('"');
      if (idx !== -1) {
        const value: string = data.substring(0, idx);
        if (value) values.push(Math.round(parseFloat(value) * 1e8) / 1e8); // Round to the 8th decimal
      }
    }
  
    if (values.length) {
      const finalRate = values[0];
      if (!isNaN(finalRate)) {
        return { source: 'Google Finance', success: true, rate: finalRate };
      }
    }
